...
const EMPTY_SKILL_GAP = { skill: "", severity: "medium" }
...
const FEATURES = [
  {
    ...
    description: 'Get AI-generated technical & behavioral questions with answers and a preparation roadmap.',
  ...
  },
  {
    ...
    description: '5 basic-level DSA & programming questions. No code editor, just short answers.',
  ...
  },
  {
    ...
    icon: '📄',
    title: 'AI Resume',
    description: 'Download a professionally tailored, ATS-friendly resume generated from the current saved session.',
...
const createFormState = (session) => ({
  title: session?.title || "",
  matchScore: String(session?.matchScore ?? 0),
  selfDescription: session?.selfDescription || "",
  jobDescription: session?.jobDescription || "",
  resume: session?.resume || "",
  skillGaps: Array.isArray(session?.skillGaps) && session.skillGaps.length > 0
    ? session.skillGaps.map((gap) => ({
        skill: gap.skill || "",
...
const buildSessionPayload = (source) => ({
  ...
  matchScore: Number(source.matchScore),
...
const Dashboard = () => {
  ...
  const handleSkillGapChange = (index, field, value) => {
    setForm((current) => ({
      ...current,
      skillGaps: current.skillGaps.map((gap, gapIndex) => (
        gapIndex === index ? { ...gap, [field]: value } : gap
      ))
    }))
  }
  ...
  const handleAddSkillGap = () => {
    setForm((current) => ({
      ...current,
      skillGaps: [...current.skillGaps, { ...EMPTY_SKILL_GAP }]
    }))
  }

  const handleRemoveSkillGap = (index) => {
    setForm((current) => {
      const nextSkillGaps = current.skillGaps.filter((_, gapIndex) => gapIndex !== index)

      return {
        ...current,
        skillGaps: nextSkillGaps.length > 0 ? nextSkillGaps : [{ ...EMPTY_SKILL_GAP }]
      }
    })
  }
  ...
  const handleSave = async () => {
    ...
    if (!payload.title || !payload.resume || !payload.selfDescription || !payload.jobDescription) {
      addToast({ message: "Title, resume, self description, and job description are required.", type: "error" })
      return
    }

    if (!Number.isFinite(payload.matchScore) || payload.matchScore < 0 || payload.matchScore > 100) {
      addToast({ message: "Match score must be a number between 0 and 100.", type: "error" })
      return
    }
    ...
    try {
      ...
      setForm(createFormState(data.session))
      addToast({
        message: "Session saved. Existing generated materials were cleared, regenerate them from the dashboard.",
        type: "success",
        duration: 5000
      })
    } catch (err) {
    ...
    <main className="min-h-screen bg-gray-900 text-white p-6">
      ...
      <div className="max-w-6xl mx-auto animate-fade-in-up space-y-8">
        <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
          <section className="p-6 rounded-2xl bg-white/5 backdrop-blur-xl border border-white/10 shadow-lg">
            <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
              <div>
                <p className="text-gray-500 text-xs uppercase tracking-wider mb-1">Session</p>
                ...
                <p className="text-gray-400 text-sm mt-4 max-w-xl">
                  Edit the extracted session data here. Saving clears previously generated interview reports, tests, and cached resume HTML so new outputs stay aligned with the latest content.
                </p>
              </div>

              <div className="grid grid-cols-2 gap-3 md:min-w-[220px]">
                <div className="rounded-2xl border border-yellow-400/20 bg-yellow-500/10 p-4 text-center">
                  <p className="text-gray-400 text-xs mb-1">Match Score</p>
                  <span className="text-3xl font-bold text-yellow-300">{session.matchScore}%</span>
                </div>
                <div className="rounded-2xl border border-blue-400/20 bg-blue-500/10 p-4 text-center">
                  <p className="text-gray-400 text-xs mb-1">Resume Cache</p>
                  <span className="text-sm font-medium text-blue-300">
                    {session.refinedResumeHtml ? "Ready" : "Will build on demand"}
                  </span>
                </div>
              </div>
            </div>
          ...
          <section className="p-6 rounded-2xl bg-white/5 backdrop-blur-xl border border-white/10 shadow-lg">
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="text-gray-500 text-xs uppercase tracking-wider mb-1">Save Flow</p>
            ...
            <div className="mt-6 grid gap-5">
              <label className="block">
                <span className="mb-2 block text-sm text-gray-400">Profile Title</span>
                <input
                  type="text"
                  value={form.title}
              ...
              <div className="grid gap-5 md:grid-cols-[180px_1fr]">
                <label className="block">
                  <span className="mb-2 block text-sm text-gray-400">Match Score</span>
                  <input
                    type="number"
                    min="0"
                    max="100"
                ...
                <div className="block">
                  <span className="mb-2 block text-sm text-gray-400">Skill Gaps</span>
                  <div className="space-y-3">
                    {form.skillGaps.map((gap, index) => (
                      <div key={`${index}-${gap.skill}-${gap.severity}`} className="grid gap-3 rounded-xl border border-white/10 bg-white/5 p-3 md:grid-cols-[1fr_140px_auto]">
                        <input
                          type="text"
                          value={gap.skill}
                          placeholder="Skill name"
                          onChange={(e) => handleSkillGapChange(index, "skill", e.target.value)}
                          className="w-full rounded-lg border border-white/10 bg-black/10 px-3 py-2 text-white outline-none transition-all focus:border-white/25"
                        />
                        <select
                          value={gap.severity}
                          onChange={(e) => handleSkillGapChange(index, "severity", e.target.value)}
                          className="rounded-lg border border-white/10 bg-black/20 px-3 py-2 text-white outline-none transition-all focus:border-white/25"
                        >
                        ...
                        <button
                          ...
                          className="rounded-lg border border-red-400/30 bg-red-500/10 px-3 py-2 text-sm text-red-300 transition-all hover:bg-red-500/20"
                        >
                          Remove
                        </button>
                      </div>
                    ...
                    <button
                      ...
                      className="rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-sm text-gray-300 transition-all hover:bg-white/10"
                    >
                      Add Skill Gap
                    </button>
                  </div>
        ...
        <section>
          <div className="mb-4 flex items-center justify-between">
            <div>
              <p className="text-gray-500 text-xs uppercase tracking-wider mb-1">AI Tools</p>
